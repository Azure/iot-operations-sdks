...
use super::delete_thing_input_arguments::DeleteThingInputArguments;
use super::delete_thing_response_schema::DeleteThingResponseSchema;
use super::sample_error::SampleError;

pub type DeleteThingRequest = rpc_command::invoker::Request<DeleteThingInputArguments>;
pub type DeleteThingResponse = rpc_command::invoker::Response<EmptyJson>;
pub type DeleteThingResponseError = rpc_command::invoker::Response<SampleError>;
pub type DeleteThingRequestBuilderError = rpc_command::invoker::RequestBuilderError;

#[derive(Default)]
/// Builder for [`DeleteThingRequest`]
pub struct DeleteThingRequestBuilder {
    inner_builder: rpc_command::invoker::RequestBuilder<DeleteThingInputArguments>,
    topic_tokens: HashMap<String, String>,
}

impl DeleteThingRequestBuilder {
    ...
    pub fn payload(
        &mut self,
        payload: DeleteThingInputArguments,
    ) -> Result<&mut Self, AIOProtocolError> {
        self.inner_builder.payload(payload)?;
        Ok(self)
    }

    /// Builds a new `DeleteThingRequest`
    ///
    /// # Errors
    /// If a required field has not been initialized
    #[allow(clippy::missing_panics_doc)] // The panic is not possible
    pub fn build(&mut self) -> Result<DeleteThingRequest, DeleteThingRequestBuilderError> {
...
/// Command Invoker for `deleteThing`
pub struct DeleteThingActionInvoker(
    rpc_command::Invoker<DeleteThingInputArguments, DeleteThingResponseSchema>,
);

impl DeleteThingActionInvoker
{
    /// Creates a new [`DeleteThingActionInvoker`]
    ///
    /// # Panics
    ...
    pub fn new(application_context: ApplicationContext, client: SessionManagedClient, options: &CommandInvokerOptions) -> Self {
        ...
        let invoker_options = invoker_options_builder
            .request_topic_pattern("sample/delete/{ex:thingId}")
            .command_name("deleteThing")
    ...
    /// Invokes the [`DeleteThingRequest`]
    ///
    /// # Errors
    ...
    ) -> Result<Result<DeleteThingResponse, DeleteThingResponseError>, AIOProtocolError> {
        let response = self.0.invoke(request).await;
        match response {
            Ok(response) => {
                if let Some(error) = response.payload.error {
                    Ok(Err(DeleteThingResponseError {
                        payload: error,
    ...
    /// Shutdown the [`DeleteThingActionInvoker`]. Unsubscribes from the response topic and cancels the receiver loop to drop the receiver and to prevent the task from looping indefinitely.
    ///
