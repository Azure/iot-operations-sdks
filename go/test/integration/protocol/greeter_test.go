...
func TestSayHelloWithDelay(t *testing.T) {
	...
	executor, err := protocol.NewCommandExecutor(server, encReq, encRes, topic,
		func(
			...
			response := envoy.HelloResponse{
				Message: fmt.Sprintf("Hello %s after %s", cr.Payload.HelloRequest.Name, cr.Payload.Delay),
			}
	...
	expected := fmt.Sprintf("Hello %s after %s", req.HelloRequest.Name, req.Delay)
	require.Equal(t, expected, res.Payload.Message)
}
func TestSayHelloWithDelayZeroThrows(t *testing.T) {
    ...
    _, err = invoker.Invoke(ctx, req, protocol.WithTopicTokens{"executorId": server.ID()})
    require.Error(t, err)
