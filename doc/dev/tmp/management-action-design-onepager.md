...
`ManagementActionNotification` with its 4 variants (Updated, UpdatedWithNewExecutor, AssetUpdated, Deleted) is internal to `ManagementActionConnectorWorker`. Users never see notifications at all — the base class consumes them on a separate loop and reacts (swaps executors, re-validates config, pauses health). The user's handler is called independently, once per incoming MQTT RPC request on the action's topic. Lifecycle changes are signaled to in-flight handler calls via the `CancellationToken` passed to `HandleAsync`; when an action is replaced or deleted, that token is cancelled and the handler is later `DisposeAsync`'d.
