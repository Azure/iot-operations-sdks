...
| topic-token-map | drive | no | map from string to string | { } | A map from topic tokens to replacement values. |
| response-topic-pattern | drive | no | string or null | { } | The response topic pattern to attach to the invocation request, as an alternative to using prefix/suffix. |
