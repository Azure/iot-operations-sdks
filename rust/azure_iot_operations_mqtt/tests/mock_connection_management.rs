...
async fn connect_disconnect() {
    ...
    assert_eq!(
        connect.password,
        Some(
            Bytes::from(connection_settings.password().clone().unwrap())
                .as_ref()
                .into()
        )
    );
    ...
    assert_eq!(
        connect.client_id,
        Some(connection_settings.client_id().as_str().into())
    );
    assert!(connect.clean_start);
    assert_eq!(
        connect.keep_alive,
        mqtt_proto::KeepAlive::Duration(NonZeroU16::new(120).unwrap())
    );

    let expected_properties: mqtt_proto::ConnectOtherProperties<Bytes> =
