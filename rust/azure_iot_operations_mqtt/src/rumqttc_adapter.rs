...
impl TryFrom<MqttConnectionSettings> for azure_mqtt::packet::ConnectProperties {
    ...
    fn try_from(value: MqttConnectionSettings) -> Result<Self, Self::Error> {
        let session_expiry_secs = value.session_expiry.as_secs()
            .try_into()
            .map_err(|_| format!("Session expiry duration {} seconds exceeds u32::MAX", value.session_expiry.as_secs()))?;
        ...
        Ok(
            azure_mqtt::packet::ConnectProperties {
...
fn az_mqtt_tls_config(
    ...
    let ca_trust_bundle = if let Some(ca_file) = ca_file {
        ...
        return Err(anyhow::anyhow!("CA file is required for azure_mqtt TLS configuration"));
    };
...
impl TryFrom<MqttConnectionSettings> for azure_mqtt::client::ConnectionTransportConfig {
    ...
    fn try_from(value: MqttConnectionSettings) -> Result<Self, Self::Error> {
        if value.use_tls {
            let (client_cert, ca_trust_bundle) = az_mqtt_tls_config(value.ca_file, value.cert_file, value.key_file, value.key_password_file)?;
            let config = azure_mqtt::client::ConnectionTransportTlsConfig::new(client_cert, ca_trust_bundle)?;
            ...
            Ok(
                azure_mqtt::client::ConnectionTransportConfig::Tcp { hostname: value.hostname, port: value.tcp_port }
            )
        }
    }
}
