...
use super::list_versions_with_label_output_arguments::ListVersionsWithLabelOutputArguments;
use super::list_versions_with_label_response_schema::ListVersionsWithLabelResponseSchema;

pub type ListVersionsWithLabelRequest =
    rpc_command::invoker::Request<ListVersionsWithLabelInputArguments>;
pub type ListVersionsWithLabelResponse =
    rpc_command::invoker::Response<ListVersionsWithLabelOutputArguments>;
pub type ListVersionsWithLabelResponseError = rpc_command::invoker::Response<EdgeRegistryError>;
pub type ListVersionsWithLabelRequestBuilderError = rpc_command::invoker::RequestBuilderError;

#[derive(Default)]
/// Builder for [`ListVersionsWithLabelRequest`]
pub struct ListVersionsWithLabelRequestBuilder {
    inner_builder: rpc_command::invoker::RequestBuilder<ListVersionsWithLabelInputArguments>,
    topic_tokens: HashMap<String, String>,
}

impl ListVersionsWithLabelRequestBuilder {
    ...
    pub fn payload(
        &mut self,
        payload: ListVersionsWithLabelInputArguments,
    ) -> Result<&mut Self, AIOProtocolError> {
        self.inner_builder.payload(payload)?;
        Ok(self)
    }

    /// Builds a new `ListVersionsWithLabelRequest`
    ///
    /// # Errors
    ...
    pub fn build(
        &mut self,
    ) -> Result<ListVersionsWithLabelRequest, ListVersionsWithLabelRequestBuilderError> {
...
/// Command Invoker for `listVersionsWithLabel`
pub struct ListVersionsWithLabelActionInvoker(
    rpc_command::Invoker<ListVersionsWithLabelInputArguments, ListVersionsWithLabelResponseSchema>,
);

impl ListVersionsWithLabelActionInvoker {
    /// Creates a new [`ListVersionsWithLabelActionInvoker`]
    ///
    /// # Panics
    ...
    pub fn new(
        ...
        let invoker_options = invoker_options_builder
            .request_topic_pattern("aio/registry/listWithLabel/versions")
            .command_name("listVersionsWithLabel")
    ...
    /// Invokes the [`ListVersionsWithLabelRequest`]
    ///
    /// # Errors
    /// [`AIOProtocolError`] if there is a failure invoking the request
    pub async fn invoke(
        &self,
        request: ListVersionsWithLabelRequest,
    ) -> Result<
        Result<ListVersionsWithLabelResponse, ListVersionsWithLabelResponseError>,
        AIOProtocolError,
    > {
        ...
        match response {
            Ok(response) => {
                if let Some(error) = response.payload.error {
                    Ok(Err(ListVersionsWithLabelResponseError {
                        payload: error,
                    ...
                    Ok(Ok(ListVersionsWithLabelResponse {
                        payload: response
                            .payload
                            .output
                            .ok_or(ListVersionsWithLabelActionInvoker::get_err("_output"))?,
                        content_type: response.content_type,
                        format_indicator: response.format_indicator,
                        custom_user_data: response.custom_user_data,
                        timestamp: response.timestamp,
    ...
    /// Shutdown the [`ListVersionsWithLabelActionInvoker`]. Unsubscribes from the response topic and cancels the receiver loop to drop the receiver and to prevent the task from looping indefinitely.
    ///
    ...
    fn get_err(field_name: &str) -> AIOProtocolError {
        AIOProtocolError {
            ...
            property_name: None,
            property_value: None,
            command_name: Some("listVersionsWithLabel".to_string()),
