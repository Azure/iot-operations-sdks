...
#[derive(Default)]
/// Builder for [`DeleteVersionRequest`]
pub struct DeleteVersionRequestBuilder {
    inner_builder: rpc_command::invoker::RequestBuilder<DeleteVersionInputArguments>,
...
impl DeleteVersionRequestBuilder {
    ...
    pub fn payload(
        &mut self,
        payload: DeleteVersionInputArguments,
    ) -> Result<&mut Self, AIOProtocolError> {
        self.inner_builder.payload(payload)?;
        Ok(self)
    }

    /// Builds a new `DeleteVersionRequest`
    ///
    /// # Errors
    /// If a required field has not been initialized
    #[allow(clippy::missing_panics_doc)] // The panic is not possible
    pub fn build(&mut self) -> Result<DeleteVersionRequest, DeleteVersionRequestBuilderError> {
...
/// Command Invoker for `deleteVersion`
pub struct DeleteVersionActionInvoker(
    rpc_command::Invoker<DeleteVersionInputArguments, DeleteVersionResponseSchema>,
);

impl DeleteVersionActionInvoker {
    /// Creates a new [`DeleteVersionActionInvoker`]
    ///
    /// # Panics
    ...
    pub fn new(
        ...
        let invoker_options = invoker_options_builder
            .request_topic_pattern("aio/registry/delete/{ex:groupType}/{ex:resourceType}/{ex:resourceId}/versions/{ex:versionId}")
            .command_name("deleteVersion")
    ...
    /// Invokes the [`DeleteVersionRequest`]
    ///
    /// # Errors
    ...
    ) -> Result<Result<DeleteVersionResponse, DeleteVersionResponseError>, AIOProtocolError> {
        let response = self.0.invoke(request).await;
        match response {
            Ok(response) => {
                if let Some(error) = response.payload.error {
                    Ok(Err(DeleteVersionResponseError {
                        payload: error,
    ...
    /// Shutdown the [`DeleteVersionActionInvoker`]. Unsubscribes from the response topic and cancels the receiver loop to drop the receiver and to prevent the task from looping indefinitely.
    ///
