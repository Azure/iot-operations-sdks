...
use super::list_schema_versions_with_label_input_arguments::ListSchemaVersionsWithLabelInputArguments;
use super::list_schema_versions_with_label_output_arguments::ListSchemaVersionsWithLabelOutputArguments;
use super::list_schema_versions_with_label_response_schema::ListSchemaVersionsWithLabelResponseSchema;
use super::schema_extension_error::SchemaExtensionError;

pub type ListSchemaVersionsWithLabelRequest =
    rpc_command::invoker::Request<ListSchemaVersionsWithLabelInputArguments>;
pub type ListSchemaVersionsWithLabelResponse =
    rpc_command::invoker::Response<ListSchemaVersionsWithLabelOutputArguments>;
pub type ListSchemaVersionsWithLabelResponseError =
    rpc_command::invoker::Response<SchemaExtensionError>;
pub type ListSchemaVersionsWithLabelRequestBuilderError = rpc_command::invoker::RequestBuilderError;

#[derive(Default)]
/// Builder for [`ListSchemaVersionsWithLabelRequest`]
pub struct ListSchemaVersionsWithLabelRequestBuilder {
    inner_builder: rpc_command::invoker::RequestBuilder<ListSchemaVersionsWithLabelInputArguments>,
    topic_tokens: HashMap<String, String>,
}

impl ListSchemaVersionsWithLabelRequestBuilder {
    ...
    pub fn payload(
        &mut self,
        payload: ListSchemaVersionsWithLabelInputArguments,
    ) -> Result<&mut Self, AIOProtocolError> {
        self.inner_builder.payload(payload)?;
        Ok(self)
    }

    /// Builds a new `ListSchemaVersionsWithLabelRequest`
    ///
    /// # Errors
    ...
    pub fn build(
        &mut self,
    ) -> Result<ListSchemaVersionsWithLabelRequest, ListSchemaVersionsWithLabelRequestBuilderError>
    {
...
/// Command Invoker for `listSchemaVersionsWithLabel`
pub struct ListSchemaVersionsWithLabelActionInvoker(
    rpc_command::Invoker<
        ListSchemaVersionsWithLabelInputArguments,
        ListSchemaVersionsWithLabelResponseSchema,
    >,
);

impl ListSchemaVersionsWithLabelActionInvoker {
    /// Creates a new [`ListSchemaVersionsWithLabelActionInvoker`]
    ///
    /// # Panics
    ...
    pub fn new(
        ...
        let invoker_options = invoker_options_builder
            .request_topic_pattern("aio/registry/schemaExtension/listWithLabel/versions")
            .command_name("listSchemaVersionsWithLabel")
    ...
    /// Invokes the [`ListSchemaVersionsWithLabelRequest`]
    ///
    /// # Errors
    /// [`AIOProtocolError`] if there is a failure invoking the request
    pub async fn invoke(
        &self,
        request: ListSchemaVersionsWithLabelRequest,
    ) -> Result<
        Result<ListSchemaVersionsWithLabelResponse, ListSchemaVersionsWithLabelResponseError>,
        AIOProtocolError,
    > {
        ...
        match response {
            Ok(response) => {
                if let Some(error) = response.payload.error {
                    Ok(Err(ListSchemaVersionsWithLabelResponseError {
                        payload: error,
                    ...
                    Ok(Ok(ListSchemaVersionsWithLabelResponse {
                        payload: response
                            .payload
                            .output
                            .ok_or(ListSchemaVersionsWithLabelActionInvoker::get_err("_output"))?,
                        content_type: response.content_type,
                        format_indicator: response.format_indicator,
                        custom_user_data: response.custom_user_data,
                        timestamp: response.timestamp,
    ...
    /// Shutdown the [`ListSchemaVersionsWithLabelActionInvoker`]. Unsubscribes from the response topic and cancels the receiver loop to drop the receiver and to prevent the task from looping indefinitely.
    ///
    ...
    fn get_err(field_name: &str) -> AIOProtocolError {
        AIOProtocolError {
            ...
            property_name: None,
            property_value: None,
            command_name: Some("listSchemaVersionsWithLabel".to_string()),
