...
impl ListGroupsActionInvoker {
    ...
    ) -> Result<Result<ListGroupsResponse, ListGroupsResponseError>, AIOProtocolError> {
        let response = self.0.invoke(request).await;
        match response {
            Ok(response) => {
                if let Some(error) = response.payload.error {
                    Ok(Err(ListGroupsResponseError {
                        payload: error,
                    ...
                    Ok(Ok(ListGroupsResponse {
                        payload: ListGroupsOutputArguments {
                            ids: response
                                .payload
                                .ids
                                .ok_or(ListGroupsActionInvoker::get_err("ids"))?,
                        },
                        content_type: response.content_type,
                        format_indicator: response.format_indicator,
                        custom_user_data: response.custom_user_data,
    ...
    /// Shutdown the [`ListGroupsActionInvoker`]. Unsubscribes from the response topic and cancels the receiver loop to drop the receiver and to prevent the task from looping indefinitely.
    ///
    ...
    fn get_err(field_name: &str) -> AIOProtocolError {
        AIOProtocolError {
            ...
            property_name: None,
            property_value: None,
            command_name: Some("listGroups".to_string()),
