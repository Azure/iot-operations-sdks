...
use super::get_schema_version_input_arguments::GetSchemaVersionInputArguments;
use super::get_schema_version_output_arguments::GetSchemaVersionOutputArguments;
use super::get_schema_version_response_schema::GetSchemaVersionResponseSchema;
use super::schema_extension_error::SchemaExtensionError;

pub type GetSchemaVersionRequest = rpc_command::invoker::Request<GetSchemaVersionInputArguments>;
pub type GetSchemaVersionResponse = rpc_command::invoker::Response<GetSchemaVersionOutputArguments>;
pub type GetSchemaVersionResponseError = rpc_command::invoker::Response<SchemaExtensionError>;
pub type GetSchemaVersionRequestBuilderError = rpc_command::invoker::RequestBuilderError;

#[derive(Default)]
/// Builder for [`GetSchemaVersionRequest`]
pub struct GetSchemaVersionRequestBuilder {
    inner_builder: rpc_command::invoker::RequestBuilder<GetSchemaVersionInputArguments>,
    topic_tokens: HashMap<String, String>,
}

impl GetSchemaVersionRequestBuilder {
    ...
    pub fn payload(
        &mut self,
        payload: GetSchemaVersionInputArguments,
    ) -> Result<&mut Self, AIOProtocolError> {
        self.inner_builder.payload(payload)?;
        Ok(self)
    }

    /// Builds a new `GetSchemaVersionRequest`
    ///
    /// # Errors
    ...
    pub fn build(
        &mut self,
    ) -> Result<GetSchemaVersionRequest, GetSchemaVersionRequestBuilderError> {
...
/// Command Invoker for `getSchemaVersion`
pub struct GetSchemaVersionActionInvoker(
    rpc_command::Invoker<GetSchemaVersionInputArguments, GetSchemaVersionResponseSchema>,
);

impl GetSchemaVersionActionInvoker {
    /// Creates a new [`GetSchemaVersionActionInvoker`]
    ///
    /// # Panics
    ...
    /// Invokes the [`GetSchemaVersionRequest`]
    ///
    /// # Errors
    ...
    ) -> Result<Result<GetSchemaVersionResponse, GetSchemaVersionResponseError>, AIOProtocolError>
    {
        let response = self.0.invoke(request).await;
        match response {
            Ok(response) => {
                if let Some(error) = response.payload.error {
                    Ok(Err(GetSchemaVersionResponseError {
                        payload: error,
                    ...
                    Ok(Ok(GetSchemaVersionResponse {
                        payload: response
                            .payload
                            .output
                            .ok_or(GetSchemaVersionActionInvoker::get_err("_output"))?,
                        content_type: response.content_type,
                        format_indicator: response.format_indicator,
                        custom_user_data: response.custom_user_data,
                        timestamp: response.timestamp,
    ...
    /// Shutdown the [`GetSchemaVersionActionInvoker`]. Unsubscribes from the response topic and cancels the receiver loop to drop the receiver and to prevent the task from looping indefinitely.
    ///
    ...
    fn get_err(field_name: &str) -> AIOProtocolError {
        AIOProtocolError {
            ...
            property_name: None,
            property_value: None,
            command_name: Some("getSchemaVersion".to_string()),
