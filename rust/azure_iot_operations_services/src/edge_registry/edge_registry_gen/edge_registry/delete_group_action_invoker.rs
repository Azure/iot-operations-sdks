...
#[derive(Default)]
/// Builder for [`DeleteGroupRequest`]
pub struct DeleteGroupRequestBuilder {
    inner_builder: rpc_command::invoker::RequestBuilder<DeleteGroupInputArguments>,
...
impl DeleteGroupRequestBuilder {
    ...
    pub fn payload(
        &mut self,
        payload: DeleteGroupInputArguments,
    ) -> Result<&mut Self, AIOProtocolError> {
        self.inner_builder.payload(payload)?;
        Ok(self)
    }

    /// Builds a new `DeleteGroupRequest`
    ///
    /// # Errors
    /// If a required field has not been initialized
    #[allow(clippy::missing_panics_doc)] // The panic is not possible
    pub fn build(&mut self) -> Result<DeleteGroupRequest, DeleteGroupRequestBuilderError> {
...
/// Command Invoker for `deleteGroup`
pub struct DeleteGroupActionInvoker(
    rpc_command::Invoker<DeleteGroupInputArguments, DeleteGroupResponseSchema>,
);

impl DeleteGroupActionInvoker {
    /// Creates a new [`DeleteGroupActionInvoker`]
    ///
    /// # Panics
    ...
    pub fn new(
        ...
        let invoker_options = invoker_options_builder
            .request_topic_pattern("aio/registry/delete/{ex:groupType}/group")
            .command_name("deleteGroup")
    ...
    /// Invokes the [`DeleteGroupRequest`]
    ///
    /// # Errors
    ...
    ) -> Result<Result<DeleteGroupResponse, DeleteGroupResponseError>, AIOProtocolError> {
        let response = self.0.invoke(request).await;
        match response {
            Ok(response) => {
                if let Some(error) = response.payload.error {
                    Ok(Err(DeleteGroupResponseError {
                        payload: error,
    ...
    /// Shutdown the [`DeleteGroupActionInvoker`]. Unsubscribes from the response topic and cancels the receiver loop to drop the receiver and to prevent the task from looping indefinitely.
    ///
