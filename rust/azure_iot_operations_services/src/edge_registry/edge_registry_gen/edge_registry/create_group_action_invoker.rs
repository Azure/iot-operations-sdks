...
use super::create_group_input_arguments::CreateGroupInputArguments;
use super::create_group_output_arguments::CreateGroupOutputArguments;
use super::create_group_response_schema::CreateGroupResponseSchema;
use super::edge_registry_error::EdgeRegistryError;

pub type CreateGroupRequest = rpc_command::invoker::Request<CreateGroupInputArguments>;
pub type CreateGroupResponse = rpc_command::invoker::Response<CreateGroupOutputArguments>;
pub type CreateGroupResponseError = rpc_command::invoker::Response<EdgeRegistryError>;
pub type CreateGroupRequestBuilderError = rpc_command::invoker::RequestBuilderError;

#[derive(Default)]
/// Builder for [`CreateGroupRequest`]
pub struct CreateGroupRequestBuilder {
    inner_builder: rpc_command::invoker::RequestBuilder<CreateGroupInputArguments>,
...
impl CreateGroupRequestBuilder {
    ...
    pub fn payload(
        &mut self,
        payload: CreateGroupInputArguments,
    ) -> Result<&mut Self, AIOProtocolError> {
        self.inner_builder.payload(payload)?;
        Ok(self)
    }

    /// Builds a new `CreateGroupRequest`
    ///
    /// # Errors
    /// If a required field has not been initialized
    #[allow(clippy::missing_panics_doc)] // The panic is not possible
    pub fn build(&mut self) -> Result<CreateGroupRequest, CreateGroupRequestBuilderError> {
...
/// Command Invoker for `createGroup`
pub struct CreateGroupActionInvoker(
    rpc_command::Invoker<CreateGroupInputArguments, CreateGroupResponseSchema>,
);

impl CreateGroupActionInvoker {
    /// Creates a new [`CreateGroupActionInvoker`]
    ///
    /// # Panics
    ...
    pub fn new(
        ...
        let invoker_options = invoker_options_builder
            .request_topic_pattern("aio/registry/create/{ex:groupType}/group")
    ...
    /// Invokes the [`CreateGroupRequest`]
    ///
    /// # Errors
    ...
    ) -> Result<Result<CreateGroupResponse, CreateGroupResponseError>, AIOProtocolError> {
        let response = self.0.invoke(request).await;
        match response {
            Ok(response) => {
                if let Some(error) = response.payload.error {
                    Ok(Err(CreateGroupResponseError {
                        payload: error,
                    ...
                    Ok(Ok(CreateGroupResponse {
                        payload: response
                            .payload
                            .output
                            .ok_or(CreateGroupActionInvoker::get_err("_output"))?,
                        content_type: response.content_type,
                        format_indicator: response.format_indicator,
                        custom_user_data: response.custom_user_data,
                        timestamp: response.timestamp,
    ...
    /// Shutdown the [`CreateGroupActionInvoker`]. Unsubscribes from the response topic and cancels the receiver loop to drop the receiver and to prevent the task from looping indefinitely.
    ///
    ...
    fn get_err(field_name: &str) -> AIOProtocolError {
        AIOProtocolError {
            ...
            property_name: None,
            property_value: None,
            command_name: Some("createGroup".to_string()),
