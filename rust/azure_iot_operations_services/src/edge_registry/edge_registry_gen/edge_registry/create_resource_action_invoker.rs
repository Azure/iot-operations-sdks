...
use super::create_resource_input_arguments::CreateResourceInputArguments;
use super::create_resource_output_arguments::CreateResourceOutputArguments;
use super::create_resource_response_schema::CreateResourceResponseSchema;
use super::edge_registry_error::EdgeRegistryError;

pub type CreateResourceRequest = rpc_command::invoker::Request<CreateResourceInputArguments>;
pub type CreateResourceResponse = rpc_command::invoker::Response<CreateResourceOutputArguments>;
pub type CreateResourceResponseError = rpc_command::invoker::Response<EdgeRegistryError>;
pub type CreateResourceRequestBuilderError = rpc_command::invoker::RequestBuilderError;

#[derive(Default)]
/// Builder for [`CreateResourceRequest`]
pub struct CreateResourceRequestBuilder {
    inner_builder: rpc_command::invoker::RequestBuilder<CreateResourceInputArguments>,
    topic_tokens: HashMap<String, String>,
}

impl CreateResourceRequestBuilder {
    ...
    pub fn payload(
        &mut self,
        payload: CreateResourceInputArguments,
    ) -> Result<&mut Self, AIOProtocolError> {
        self.inner_builder.payload(payload)?;
        Ok(self)
    }

    /// Builds a new `CreateResourceRequest`
    ///
    /// # Errors
    /// If a required field has not been initialized
    #[allow(clippy::missing_panics_doc)] // The panic is not possible
    pub fn build(&mut self) -> Result<CreateResourceRequest, CreateResourceRequestBuilderError> {
...
/// Command Invoker for `createResource`
pub struct CreateResourceActionInvoker(
    rpc_command::Invoker<CreateResourceInputArguments, CreateResourceResponseSchema>,
);

impl CreateResourceActionInvoker {
    /// Creates a new [`CreateResourceActionInvoker`]
    ///
    /// # Panics
    ...
    pub fn new(
        ...
        let invoker_options = invoker_options_builder
            .request_topic_pattern(
                "aio/registry/create/{ex:groupType}/{ex:resourceType}/{ex:resourceId}",
            )
            .command_name("createResource")
    ...
    /// Invokes the [`CreateResourceRequest`]
    ///
    /// # Errors
    ...
    ) -> Result<Result<CreateResourceResponse, CreateResourceResponseError>, AIOProtocolError> {
        let response = self.0.invoke(request).await;
        match response {
            Ok(response) => {
                if let Some(error) = response.payload.error {
                    Ok(Err(CreateResourceResponseError {
                        payload: error,
                    ...
                    Ok(Ok(CreateResourceResponse {
                        payload: response
                            .payload
                            .output
                            .ok_or(CreateResourceActionInvoker::get_err("_output"))?,
                        content_type: response.content_type,
                        format_indicator: response.format_indicator,
                        custom_user_data: response.custom_user_data,
                        timestamp: response.timestamp,
    ...
    /// Shutdown the [`CreateResourceActionInvoker`]. Unsubscribes from the response topic and cancels the receiver loop to drop the receiver and to prevent the task from looping indefinitely.
    ///
    ...
    fn get_err(field_name: &str) -> AIOProtocolError {
        AIOProtocolError {
            ...
            property_name: None,
            property_value: None,
            command_name: Some("createResource".to_string()),
