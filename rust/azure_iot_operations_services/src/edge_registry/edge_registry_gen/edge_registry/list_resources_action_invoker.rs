...
use super::list_resources_input_arguments::ListResourcesInputArguments;
use super::list_resources_output_arguments::ListResourcesOutputArguments;
use super::list_resources_response_schema::ListResourcesResponseSchema;

pub type ListResourcesRequest = rpc_command::invoker::Request<ListResourcesInputArguments>;
pub type ListResourcesResponse = rpc_command::invoker::Response<ListResourcesOutputArguments>;
pub type ListResourcesResponseError = rpc_command::invoker::Response<EdgeRegistryError>;
pub type ListResourcesRequestBuilderError = rpc_command::invoker::RequestBuilderError;

#[derive(Default)]
/// Builder for [`ListResourcesRequest`]
pub struct ListResourcesRequestBuilder {
    inner_builder: rpc_command::invoker::RequestBuilder<ListResourcesInputArguments>,
    topic_tokens: HashMap<String, String>,
}

impl ListResourcesRequestBuilder {
    ...
    pub fn payload(
        &mut self,
        payload: ListResourcesInputArguments,
    ) -> Result<&mut Self, AIOProtocolError> {
        self.inner_builder.payload(payload)?;
        Ok(self)
    }

    /// Builds a new `ListResourcesRequest`
    ///
    /// # Errors
    /// If a required field has not been initialized
    #[allow(clippy::missing_panics_doc)] // The panic is not possible
    pub fn build(&mut self) -> Result<ListResourcesRequest, ListResourcesRequestBuilderError> {
...
/// Command Invoker for `listResources`
pub struct ListResourcesActionInvoker(
    rpc_command::Invoker<ListResourcesInputArguments, ListResourcesResponseSchema>,
);

impl ListResourcesActionInvoker {
    /// Creates a new [`ListResourcesActionInvoker`]
    ///
    /// # Panics
    ...
    /// Invokes the [`ListResourcesRequest`]
    ///
    /// # Errors
    ...
    ) -> Result<Result<ListResourcesResponse, ListResourcesResponseError>, AIOProtocolError> {
        let response = self.0.invoke(request).await;
        match response {
            Ok(response) => {
                if let Some(error) = response.payload.error {
                    Ok(Err(ListResourcesResponseError {
                        payload: error,
    ...
    /// Shutdown the [`ListResourcesActionInvoker`]. Unsubscribes from the response topic and cancels the receiver loop to drop the receiver and to prevent the task from looping indefinitely.
    ///
    ...
    fn get_err(field_name: &str) -> AIOProtocolError {
        AIOProtocolError {
            ...
            property_name: None,
            property_value: None,
            command_name: Some("listResources".to_string()),
