...
use super::create_thing_model_version_input_arguments::CreateThingModelVersionInputArguments;
use super::create_thing_model_version_output_arguments::CreateThingModelVersionOutputArguments;
use super::create_thing_model_version_response_schema::CreateThingModelVersionResponseSchema;
use super::thing_model_extension_error::ThingModelExtensionError;

pub type CreateThingModelVersionRequest =
    rpc_command::invoker::Request<CreateThingModelVersionInputArguments>;
pub type CreateThingModelVersionResponse =
    rpc_command::invoker::Response<CreateThingModelVersionOutputArguments>;
pub type CreateThingModelVersionResponseError =
    rpc_command::invoker::Response<ThingModelExtensionError>;
pub type CreateThingModelVersionRequestBuilderError = rpc_command::invoker::RequestBuilderError;

#[derive(Default)]
/// Builder for [`CreateThingModelVersionRequest`]
pub struct CreateThingModelVersionRequestBuilder {
    inner_builder: rpc_command::invoker::RequestBuilder<CreateThingModelVersionInputArguments>,
    topic_tokens: HashMap<String, String>,
}

impl CreateThingModelVersionRequestBuilder {
    ...
    pub fn payload(
        &mut self,
        payload: CreateThingModelVersionInputArguments,
    ) -> Result<&mut Self, AIOProtocolError> {
        self.inner_builder.payload(payload)?;
        Ok(self)
    }

    /// Builds a new `CreateThingModelVersionRequest`
    ///
    /// # Errors
    ...
    pub fn build(
        &mut self,
    ) -> Result<CreateThingModelVersionRequest, CreateThingModelVersionRequestBuilderError> {
...
/// Command Invoker for `createThingModelVersion`
pub struct CreateThingModelVersionActionInvoker(
    rpc_command::Invoker<
        CreateThingModelVersionInputArguments,
        CreateThingModelVersionResponseSchema,
    >,
);

impl CreateThingModelVersionActionInvoker {
    /// Creates a new [`CreateThingModelVersionActionInvoker`]
    ///
    /// # Panics
    ...
    pub fn new(
        ...
        let invoker_options = invoker_options_builder
            .request_topic_pattern("aio/registry/thingModelExtension/create/thingmodelgroups/thingmodels/{ex:thingModelId}/versions/version")
            .command_name("createThingModelVersion")
    ...
    /// Invokes the [`CreateThingModelVersionRequest`]
    ///
    /// # Errors
    /// [`AIOProtocolError`] if there is a failure invoking the request
    pub async fn invoke(
        &self,
        request: CreateThingModelVersionRequest,
    ) -> Result<
        Result<CreateThingModelVersionResponse, CreateThingModelVersionResponseError>,
        AIOProtocolError,
    > {
        ...
        match response {
            Ok(response) => {
                if let Some(error) = response.payload.error {
                    Ok(Err(CreateThingModelVersionResponseError {
                        payload: error,
                    ...
                    Ok(Ok(CreateThingModelVersionResponse {
                        payload: response
                            .payload
                            .output
                            .ok_or(CreateThingModelVersionActionInvoker::get_err("_output"))?,
                        content_type: response.content_type,
                        format_indicator: response.format_indicator,
                        custom_user_data: response.custom_user_data,
                        timestamp: response.timestamp,
    ...
    /// Shutdown the [`CreateThingModelVersionActionInvoker`]. Unsubscribes from the response topic and cancels the receiver loop to drop the receiver and to prevent the task from looping indefinitely.
    ///
    ...
    fn get_err(field_name: &str) -> AIOProtocolError {
        AIOProtocolError {
            ...
            property_name: None,
            property_value: None,
            command_name: Some("createThingModelVersion".to_string()),
