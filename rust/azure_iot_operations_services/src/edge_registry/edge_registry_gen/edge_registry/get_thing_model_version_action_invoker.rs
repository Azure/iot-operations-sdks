...
use super::get_thing_model_version_input_arguments::GetThingModelVersionInputArguments;
use super::get_thing_model_version_output_arguments::GetThingModelVersionOutputArguments;
use super::get_thing_model_version_response_schema::GetThingModelVersionResponseSchema;
use super::thing_model_extension_error::ThingModelExtensionError;

pub type GetThingModelVersionRequest =
    rpc_command::invoker::Request<GetThingModelVersionInputArguments>;
pub type GetThingModelVersionResponse =
    rpc_command::invoker::Response<GetThingModelVersionOutputArguments>;
pub type GetThingModelVersionResponseError =
    rpc_command::invoker::Response<ThingModelExtensionError>;
pub type GetThingModelVersionRequestBuilderError = rpc_command::invoker::RequestBuilderError;

#[derive(Default)]
/// Builder for [`GetThingModelVersionRequest`]
pub struct GetThingModelVersionRequestBuilder {
    inner_builder: rpc_command::invoker::RequestBuilder<GetThingModelVersionInputArguments>,
    topic_tokens: HashMap<String, String>,
}

impl GetThingModelVersionRequestBuilder {
    ...
    pub fn payload(
        &mut self,
        payload: GetThingModelVersionInputArguments,
    ) -> Result<&mut Self, AIOProtocolError> {
        self.inner_builder.payload(payload)?;
        Ok(self)
    }

    /// Builds a new `GetThingModelVersionRequest`
    ///
    /// # Errors
    ...
    pub fn build(
        &mut self,
    ) -> Result<GetThingModelVersionRequest, GetThingModelVersionRequestBuilderError> {
...
/// Command Invoker for `getThingModelVersion`
pub struct GetThingModelVersionActionInvoker(
    rpc_command::Invoker<GetThingModelVersionInputArguments, GetThingModelVersionResponseSchema>,
);

impl GetThingModelVersionActionInvoker {
    /// Creates a new [`GetThingModelVersionActionInvoker`]
    ///
    /// # Panics
    ...
    pub fn new(
        ...
        let invoker_options = invoker_options_builder
            .request_topic_pattern("aio/registry/thingModelExtension/get/thingmodelgroups/thingmodels/{ex:thingModelId}/versions/version")
            .command_name("getThingModelVersion")
    ...
    /// Invokes the [`GetThingModelVersionRequest`]
    ///
    /// # Errors
    /// [`AIOProtocolError`] if there is a failure invoking the request
    pub async fn invoke(
        &self,
        request: GetThingModelVersionRequest,
    ) -> Result<
        Result<GetThingModelVersionResponse, GetThingModelVersionResponseError>,
        AIOProtocolError,
    > {
        ...
        match response {
            Ok(response) => {
                if let Some(error) = response.payload.error {
                    Ok(Err(GetThingModelVersionResponseError {
                        payload: error,
                    ...
                    Ok(Ok(GetThingModelVersionResponse {
                        payload: response
                            .payload
                            .output
                            .ok_or(GetThingModelVersionActionInvoker::get_err("_output"))?,
                        content_type: response.content_type,
                        format_indicator: response.format_indicator,
                        custom_user_data: response.custom_user_data,
                        timestamp: response.timestamp,
    ...
    /// Shutdown the [`GetThingModelVersionActionInvoker`]. Unsubscribes from the response topic and cancels the receiver loop to drop the receiver and to prevent the task from looping indefinitely.
    ///
    ...
    fn get_err(field_name: &str) -> AIOProtocolError {
        AIOProtocolError {
            ...
            property_name: None,
            property_value: None,
            command_name: Some("getThingModelVersion".to_string()),
