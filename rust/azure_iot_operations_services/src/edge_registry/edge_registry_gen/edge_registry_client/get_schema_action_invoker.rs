...
use super::get_schema_input_arguments::GetSchemaInputArguments;
use super::get_schema_output_arguments::GetSchemaOutputArguments;
use super::get_schema_response_schema::GetSchemaResponseSchema;
use super::schema_extension_error::SchemaExtensionError;

pub type GetSchemaRequest = rpc_command::invoker::Request<GetSchemaInputArguments>;
pub type GetSchemaResponse = rpc_command::invoker::Response<GetSchemaOutputArguments>;
pub type GetSchemaResponseError = rpc_command::invoker::Response<SchemaExtensionError>;
pub type GetSchemaRequestBuilderError = rpc_command::invoker::RequestBuilderError;

#[derive(Default)]
/// Builder for [`GetSchemaRequest`]
pub struct GetSchemaRequestBuilder {
    inner_builder: rpc_command::invoker::RequestBuilder<GetSchemaInputArguments>,
...
impl GetSchemaRequestBuilder {
    ...
    pub fn payload(
        &mut self,
        payload: GetSchemaInputArguments,
    ) -> Result<&mut Self, AIOProtocolError> {
        self.inner_builder.payload(payload)?;
        Ok(self)
    }

    /// Builds a new `GetSchemaRequest`
    ///
    /// # Errors
    /// If a required field has not been initialized
    #[allow(clippy::missing_panics_doc)] // The panic is not possible
    pub fn build(&mut self) -> Result<GetSchemaRequest, GetSchemaRequestBuilderError> {
...
/// Command Invoker for `getSchema`
pub struct GetSchemaActionInvoker(
    rpc_command::Invoker<GetSchemaInputArguments, GetSchemaResponseSchema>,
);

impl GetSchemaActionInvoker {
    /// Creates a new [`GetSchemaActionInvoker`]
    ///
    /// # Panics
    ...
    pub fn new(
        ...
        let invoker_options = invoker_options_builder
            .request_topic_pattern("adr/dtmi:ms:adr:EdgeRegistry;1/get/schema")
            .command_name("getSchema")
    ...
    ) -> Result<Result<GetSchemaResponse, GetSchemaResponseError>, AIOProtocolError> {
        let response = self.0.invoke(request).await;
        match response {
            Ok(response) => {
                if let Some(error) = response.payload.error {
                    Ok(Err(GetSchemaResponseError {
                        payload: error,
                    ...
                    Ok(Ok(GetSchemaResponse {
                        payload: response
                            .payload
                            .output
                            .ok_or(GetSchemaActionInvoker::get_err("_output"))?,
                        content_type: response.content_type,
                        format_indicator: response.format_indicator,
                        custom_user_data: response.custom_user_data,
                        timestamp: response.timestamp,
    ...
    /// Shutdown the [`GetSchemaActionInvoker`]. Unsubscribes from the response topic and cancels the receiver loop to drop the receiver and to prevent the task from looping indefinitely.
    ///
    ...
    fn get_err(field_name: &str) -> AIOProtocolError {
        AIOProtocolError {
            ...
            property_name: None,
            property_value: None,
            command_name: Some("getSchema".to_string()),
