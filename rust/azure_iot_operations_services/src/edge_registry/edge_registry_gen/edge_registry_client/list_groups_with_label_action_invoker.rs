...
use super::list_groups_with_label_input_arguments::ListGroupsWithLabelInputArguments;
use super::list_groups_with_label_output_arguments::ListGroupsWithLabelOutputArguments;
use super::list_groups_with_label_response_schema::ListGroupsWithLabelResponseSchema;

pub type ListGroupsWithLabelRequest =
    rpc_command::invoker::Request<ListGroupsWithLabelInputArguments>;
pub type ListGroupsWithLabelResponse =
    rpc_command::invoker::Response<ListGroupsWithLabelOutputArguments>;
pub type ListGroupsWithLabelResponseError = rpc_command::invoker::Response<EdgeRegistryError>;
pub type ListGroupsWithLabelRequestBuilderError = rpc_command::invoker::RequestBuilderError;

#[derive(Default)]
/// Builder for [`ListGroupsWithLabelRequest`]
pub struct ListGroupsWithLabelRequestBuilder {
    inner_builder: rpc_command::invoker::RequestBuilder<ListGroupsWithLabelInputArguments>,
    topic_tokens: HashMap<String, String>,
}

impl ListGroupsWithLabelRequestBuilder {
    ...
    pub fn payload(
        &mut self,
        payload: ListGroupsWithLabelInputArguments,
    ) -> Result<&mut Self, AIOProtocolError> {
        self.inner_builder.payload(payload)?;
        Ok(self)
    }

    /// Builds a new `ListGroupsWithLabelRequest`
    ///
    /// # Errors
    ...
    pub fn build(
        &mut self,
    ) -> Result<ListGroupsWithLabelRequest, ListGroupsWithLabelRequestBuilderError> {
...
/// Command Invoker for `listGroupsWithLabel`
pub struct ListGroupsWithLabelActionInvoker(
    rpc_command::Invoker<ListGroupsWithLabelInputArguments, ListGroupsWithLabelResponseSchema>,
);

impl ListGroupsWithLabelActionInvoker {
    /// Creates a new [`ListGroupsWithLabelActionInvoker`]
    ///
    /// # Panics
    ...
    pub fn new(
        ...
        let invoker_options = invoker_options_builder
            .request_topic_pattern("adr/dtmi:ms:adr:EdgeRegistry;1/listGroupsWithLabel")
            .command_name("listGroupsWithLabel")
    ...
    /// Invokes the [`ListGroupsWithLabelRequest`]
    ///
    /// # Errors
    /// [`AIOProtocolError`] if there is a failure invoking the request
    pub async fn invoke(
        &self,
        request: ListGroupsWithLabelRequest,
    ) -> Result<
        Result<ListGroupsWithLabelResponse, ListGroupsWithLabelResponseError>,
        AIOProtocolError,
    > {
        ...
        match response {
            Ok(response) => {
                if let Some(error) = response.payload.error {
                    Ok(Err(ListGroupsWithLabelResponseError {
                        payload: error,
                    ...
                    Ok(Ok(ListGroupsWithLabelResponse {
                        payload: ListGroupsWithLabelOutputArguments {
                            ids: response
                                .payload
                                .ids
                                .ok_or(ListGroupsWithLabelActionInvoker::get_err("ids"))?,
                        },
                        content_type: response.content_type,
                        format_indicator: response.format_indicator,
                        custom_user_data: response.custom_user_data,
    ...
    /// Shutdown the [`ListGroupsWithLabelActionInvoker`]. Unsubscribes from the response topic and cancels the receiver loop to drop the receiver and to prevent the task from looping indefinitely.
    ///
    ...
    fn get_err(field_name: &str) -> AIOProtocolError {
        AIOProtocolError {
            ...
            property_name: None,
            property_value: None,
            command_name: Some("listGroupsWithLabel".to_string()),
