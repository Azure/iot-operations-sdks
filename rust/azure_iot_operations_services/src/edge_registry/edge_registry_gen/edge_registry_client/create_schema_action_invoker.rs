...
use super::create_schema_input_arguments::CreateSchemaInputArguments;
use super::create_schema_output_arguments::CreateSchemaOutputArguments;
use super::create_schema_response_schema::CreateSchemaResponseSchema;
use super::schema_extension_error::SchemaExtensionError;

pub type CreateSchemaRequest = rpc_command::invoker::Request<CreateSchemaInputArguments>;
pub type CreateSchemaResponse = rpc_command::invoker::Response<CreateSchemaOutputArguments>;
pub type CreateSchemaResponseError = rpc_command::invoker::Response<SchemaExtensionError>;
pub type CreateSchemaRequestBuilderError = rpc_command::invoker::RequestBuilderError;

#[derive(Default)]
/// Builder for [`CreateSchemaRequest`]
pub struct CreateSchemaRequestBuilder {
    inner_builder: rpc_command::invoker::RequestBuilder<CreateSchemaInputArguments>,
    topic_tokens: HashMap<String, String>,
}

impl CreateSchemaRequestBuilder {
    ...
    pub fn payload(
        &mut self,
        payload: CreateSchemaInputArguments,
    ) -> Result<&mut Self, AIOProtocolError> {
        self.inner_builder.payload(payload)?;
        Ok(self)
    }

    /// Builds a new `CreateSchemaRequest`
    ///
    /// # Errors
    /// If a required field has not been initialized
    #[allow(clippy::missing_panics_doc)] // The panic is not possible
    pub fn build(&mut self) -> Result<CreateSchemaRequest, CreateSchemaRequestBuilderError> {
...
/// Command Invoker for `createSchema`
pub struct CreateSchemaActionInvoker(
    rpc_command::Invoker<CreateSchemaInputArguments, CreateSchemaResponseSchema>,
);

impl CreateSchemaActionInvoker {
    /// Creates a new [`CreateSchemaActionInvoker`]
    ///
    /// # Panics
    ...
    pub fn new(
        ...
        let invoker_options = invoker_options_builder
            .request_topic_pattern("adr/dtmi:ms:adr:EdgeRegistry;1/create/schema")
            .command_name("createSchema")
    ...
    /// Invokes the [`CreateSchemaRequest`]
    ///
    /// # Errors
    ...
    ) -> Result<Result<CreateSchemaResponse, CreateSchemaResponseError>, AIOProtocolError> {
        let response = self.0.invoke(request).await;
        match response {
            Ok(response) => {
                if let Some(error) = response.payload.error {
                    Ok(Err(CreateSchemaResponseError {
                        payload: error,
                    ...
                    Ok(Ok(CreateSchemaResponse {
                        payload: response
                            .payload
                            .output
                            .ok_or(CreateSchemaActionInvoker::get_err("_output"))?,
                        content_type: response.content_type,
                        format_indicator: response.format_indicator,
                        custom_user_data: response.custom_user_data,
                        timestamp: response.timestamp,
    ...
    /// Shutdown the [`CreateSchemaActionInvoker`]. Unsubscribes from the response topic and cancels the receiver loop to drop the receiver and to prevent the task from looping indefinitely.
    ///
    ...
    fn get_err(field_name: &str) -> AIOProtocolError {
        AIOProtocolError {
            ...
            property_name: None,
            property_value: None,
            command_name: Some("createSchema".to_string()),
