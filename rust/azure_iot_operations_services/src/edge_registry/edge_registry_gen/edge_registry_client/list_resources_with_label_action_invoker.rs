...
use super::list_resources_with_label_input_arguments::ListResourcesWithLabelInputArguments;
use super::list_resources_with_label_output_arguments::ListResourcesWithLabelOutputArguments;
use super::list_resources_with_label_response_schema::ListResourcesWithLabelResponseSchema;

pub type ListResourcesWithLabelRequest =
    rpc_command::invoker::Request<ListResourcesWithLabelInputArguments>;
pub type ListResourcesWithLabelResponse =
    rpc_command::invoker::Response<ListResourcesWithLabelOutputArguments>;
pub type ListResourcesWithLabelResponseError = rpc_command::invoker::Response<EdgeRegistryError>;
pub type ListResourcesWithLabelRequestBuilderError = rpc_command::invoker::RequestBuilderError;

#[derive(Default)]
/// Builder for [`ListResourcesWithLabelRequest`]
pub struct ListResourcesWithLabelRequestBuilder {
    inner_builder: rpc_command::invoker::RequestBuilder<ListResourcesWithLabelInputArguments>,
    topic_tokens: HashMap<String, String>,
}

impl ListResourcesWithLabelRequestBuilder {
    ...
    pub fn payload(
        &mut self,
        payload: ListResourcesWithLabelInputArguments,
    ) -> Result<&mut Self, AIOProtocolError> {
        self.inner_builder.payload(payload)?;
        Ok(self)
    }

    /// Builds a new `ListResourcesWithLabelRequest`
    ///
    /// # Errors
    ...
    pub fn build(
        &mut self,
    ) -> Result<ListResourcesWithLabelRequest, ListResourcesWithLabelRequestBuilderError> {
...
/// Command Invoker for `listResourcesWithLabel`
pub struct ListResourcesWithLabelActionInvoker(
    rpc_command::Invoker<
        ListResourcesWithLabelInputArguments,
        ListResourcesWithLabelResponseSchema,
    >,
);

impl ListResourcesWithLabelActionInvoker {
    /// Creates a new [`ListResourcesWithLabelActionInvoker`]
    ///
    /// # Panics
    ...
    /// Invokes the [`ListResourcesWithLabelRequest`]
    ///
    /// # Errors
    /// [`AIOProtocolError`] if there is a failure invoking the request
    pub async fn invoke(
        &self,
        request: ListResourcesWithLabelRequest,
    ) -> Result<
        Result<ListResourcesWithLabelResponse, ListResourcesWithLabelResponseError>,
        AIOProtocolError,
    > {
        ...
        match response {
            Ok(response) => {
                if let Some(error) = response.payload.error {
                    Ok(Err(ListResourcesWithLabelResponseError {
                        payload: error,
                    ...
                    Ok(Ok(ListResourcesWithLabelResponse {
                        payload: ListResourcesWithLabelOutputArguments {
                            resources: response
                                .payload
                                .resources
                                .ok_or(ListResourcesWithLabelActionInvoker::get_err("resources"))?,
                        },
                        content_type: response.content_type,
                        format_indicator: response.format_indicator,
                        custom_user_data: response.custom_user_data,
    ...
    /// Shutdown the [`ListResourcesWithLabelActionInvoker`]. Unsubscribes from the response topic and cancels the receiver loop to drop the receiver and to prevent the task from looping indefinitely.
    ///
    ...
    fn get_err(field_name: &str) -> AIOProtocolError {
        AIOProtocolError {
            ...
            property_name: None,
            property_value: None,
            command_name: Some("listResourcesWithLabel".to_string()),
