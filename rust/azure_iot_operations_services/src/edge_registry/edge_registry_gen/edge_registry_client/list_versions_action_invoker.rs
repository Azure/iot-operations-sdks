...
use super::list_versions_input_arguments::ListVersionsInputArguments;
use super::list_versions_output_arguments::ListVersionsOutputArguments;
use super::list_versions_response_schema::ListVersionsResponseSchema;

pub type ListVersionsRequest = rpc_command::invoker::Request<ListVersionsInputArguments>;
pub type ListVersionsResponse = rpc_command::invoker::Response<ListVersionsOutputArguments>;
pub type ListVersionsResponseError = rpc_command::invoker::Response<EdgeRegistryError>;
pub type ListVersionsRequestBuilderError = rpc_command::invoker::RequestBuilderError;

#[derive(Default)]
/// Builder for [`ListVersionsRequest`]
pub struct ListVersionsRequestBuilder {
    inner_builder: rpc_command::invoker::RequestBuilder<ListVersionsInputArguments>,
...
impl ListVersionsRequestBuilder {
    ...
    pub fn payload(
        &mut self,
        payload: ListVersionsInputArguments,
    ) -> Result<&mut Self, AIOProtocolError> {
        self.inner_builder.payload(payload)?;
        Ok(self)
    }

    /// Builds a new `ListVersionsRequest`
    ///
    /// # Errors
    /// If a required field has not been initialized
    #[allow(clippy::missing_panics_doc)] // The panic is not possible
    pub fn build(&mut self) -> Result<ListVersionsRequest, ListVersionsRequestBuilderError> {
...
/// Command Invoker for `listVersions`
pub struct ListVersionsActionInvoker(
    rpc_command::Invoker<ListVersionsInputArguments, ListVersionsResponseSchema>,
);

impl ListVersionsActionInvoker {
    /// Creates a new [`ListVersionsActionInvoker`]
    ///
    /// # Panics
    ...
    /// Invokes the [`ListVersionsRequest`]
    ///
    /// # Errors
    ...
    ) -> Result<Result<ListVersionsResponse, ListVersionsResponseError>, AIOProtocolError> {
        let response = self.0.invoke(request).await;
        match response {
            Ok(response) => {
                if let Some(error) = response.payload.error {
                    Ok(Err(ListVersionsResponseError {
                        payload: error,
    ...
    /// Shutdown the [`ListVersionsActionInvoker`]. Unsubscribes from the response topic and cancels the receiver loop to drop the receiver and to prevent the task from looping indefinitely.
    ///
    ...
    fn get_err(field_name: &str) -> AIOProtocolError {
        AIOProtocolError {
            ...
            property_name: None,
            property_value: None,
            command_name: Some("listVersions".to_string()),
