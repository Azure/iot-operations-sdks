...
use super::get_group_input_arguments::GetGroupInputArguments;
use super::get_group_output_arguments::GetGroupOutputArguments;
use super::get_group_response_schema::GetGroupResponseSchema;

pub type GetGroupRequest = rpc_command::invoker::Request<GetGroupInputArguments>;
pub type GetGroupResponse = rpc_command::invoker::Response<GetGroupOutputArguments>;
pub type GetGroupResponseError = rpc_command::invoker::Response<EdgeRegistryError>;
pub type GetGroupRequestBuilderError = rpc_command::invoker::RequestBuilderError;

#[derive(Default)]
/// Builder for [`GetGroupRequest`]
pub struct GetGroupRequestBuilder {
    inner_builder: rpc_command::invoker::RequestBuilder<GetGroupInputArguments>,
...
/// Command Invoker for `getGroup`
pub struct GetGroupActionInvoker(
    rpc_command::Invoker<GetGroupInputArguments, GetGroupResponseSchema>,
);

impl GetGroupActionInvoker {
    /// Creates a new [`GetGroupActionInvoker`]
    ///
    /// # Panics
    ...
    ) -> Result<Result<GetGroupResponse, GetGroupResponseError>, AIOProtocolError> {
        let response = self.0.invoke(request).await;
        match response {
            Ok(response) => {
                if let Some(error) = response.payload.error {
                    Ok(Err(GetGroupResponseError {
                        payload: error,
                    ...
                    Ok(Ok(GetGroupResponse {
                        payload: response
                            .payload
                            .output
                            .ok_or(GetGroupActionInvoker::get_err("_output"))?,
                        content_type: response.content_type,
                        format_indicator: response.format_indicator,
                        custom_user_data: response.custom_user_data,
                        timestamp: response.timestamp,
    ...
    /// Shutdown the [`GetGroupActionInvoker`]. Unsubscribes from the response topic and cancels the receiver loop to drop the receiver and to prevent the task from looping indefinitely.
    ///
    ...
    fn get_err(field_name: &str) -> AIOProtocolError {
        AIOProtocolError {
            ...
            property_name: None,
            property_value: None,
            command_name: Some("getGroup".to_string()),
