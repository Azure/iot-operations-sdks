...
use super::create_schema_version_input_arguments::CreateSchemaVersionInputArguments;
use super::create_schema_version_output_arguments::CreateSchemaVersionOutputArguments;
use super::create_schema_version_response_schema::CreateSchemaVersionResponseSchema;
use super::schema_extension_error::SchemaExtensionError;

pub type CreateSchemaVersionRequest =
    rpc_command::invoker::Request<CreateSchemaVersionInputArguments>;
pub type CreateSchemaVersionResponse =
    rpc_command::invoker::Response<CreateSchemaVersionOutputArguments>;
pub type CreateSchemaVersionResponseError = rpc_command::invoker::Response<SchemaExtensionError>;
pub type CreateSchemaVersionRequestBuilderError = rpc_command::invoker::RequestBuilderError;

#[derive(Default)]
/// Builder for [`CreateSchemaVersionRequest`]
pub struct CreateSchemaVersionRequestBuilder {
    inner_builder: rpc_command::invoker::RequestBuilder<CreateSchemaVersionInputArguments>,
    topic_tokens: HashMap<String, String>,
}

impl CreateSchemaVersionRequestBuilder {
    ...
    pub fn payload(
        &mut self,
        payload: CreateSchemaVersionInputArguments,
    ) -> Result<&mut Self, AIOProtocolError> {
        self.inner_builder.payload(payload)?;
        Ok(self)
    }

    /// Builds a new `CreateSchemaVersionRequest`
    ///
    /// # Errors
    ...
    pub fn build(
        &mut self,
    ) -> Result<CreateSchemaVersionRequest, CreateSchemaVersionRequestBuilderError> {
...
/// Command Invoker for `createSchemaVersion`
pub struct CreateSchemaVersionActionInvoker(
    rpc_command::Invoker<CreateSchemaVersionInputArguments, CreateSchemaVersionResponseSchema>,
);

impl CreateSchemaVersionActionInvoker {
    /// Creates a new [`CreateSchemaVersionActionInvoker`]
    ///
    /// # Panics
    ...
    /// Invokes the [`CreateSchemaVersionRequest`]
    ///
    /// # Errors
    ...
    pub async fn invoke(
        ...
        Result<CreateSchemaVersionResponse, CreateSchemaVersionResponseError>,
        AIOProtocolError,
    > {
        ...
        match response {
            Ok(response) => {
                if let Some(error) = response.payload.error {
                    Ok(Err(CreateSchemaVersionResponseError {
                        payload: error,
                    ...
                    Ok(Ok(CreateSchemaVersionResponse {
                        payload: response
                            .payload
                            .output
                            .ok_or(CreateSchemaVersionActionInvoker::get_err("_output"))?,
                        content_type: response.content_type,
                        format_indicator: response.format_indicator,
                        custom_user_data: response.custom_user_data,
                        timestamp: response.timestamp,
    ...
    /// Shutdown the [`CreateSchemaVersionActionInvoker`]. Unsubscribes from the response topic and cancels the receiver loop to drop the receiver and to prevent the task from looping indefinitely.
    ///
    ...
    fn get_err(field_name: &str) -> AIOProtocolError {
        AIOProtocolError {
            ...
            property_name: None,
            property_value: None,
            command_name: Some("createSchemaVersion".to_string()),
