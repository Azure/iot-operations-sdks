...
use super::list_input_arguments::ListInputArguments;
use super::list_output_arguments::ListOutputArguments;
use super::list_response_schema::ListResponseSchema;

pub type ListRequest = rpc_command::invoker::Request<ListInputArguments>;
pub type ListResponse = rpc_command::invoker::Response<ListOutputArguments>;
pub type ListResponseError = rpc_command::invoker::Response<EdgeRegistryError>;
pub type ListRequestBuilderError = rpc_command::invoker::RequestBuilderError;

#[derive(Default)]
/// Builder for [`ListRequest`]
pub struct ListRequestBuilder {
    inner_builder: rpc_command::invoker::RequestBuilder<ListInputArguments>,
...
impl ListRequestBuilder {
    ...
    pub fn payload(&mut self, payload: ListInputArguments) -> Result<&mut Self, AIOProtocolError> {
        self.inner_builder.payload(payload)?;
        Ok(self)
    }

    /// Builds a new `ListRequest`
    ///
    /// # Errors
    /// If a required field has not been initialized
    #[allow(clippy::missing_panics_doc)] // The panic is not possible
    pub fn build(&mut self) -> Result<ListRequest, ListRequestBuilderError> {
...
/// Command Invoker for `list`
pub struct ListActionInvoker(rpc_command::Invoker<ListInputArguments, ListResponseSchema>);

impl ListActionInvoker {
    /// Creates a new [`ListActionInvoker`]
    ///
    /// # Panics
    ...
    pub fn new(
        ...
        let invoker_options = invoker_options_builder
            .request_topic_pattern("adr/dtmi:ms:adr:EdgeRegistry;1/list")
            .command_name("list")
    ...
    pub async fn invoke(
        &self,
        request: ListRequest,
    ) -> Result<Result<ListResponse, ListResponseError>, AIOProtocolError> {
        let response = self.0.invoke(request).await;
        match response {
            Ok(response) => {
                if let Some(error) = response.payload.error {
                    Ok(Err(ListResponseError {
                        payload: error,
                    ...
                    Ok(Ok(ListResponse {
                        payload: ListOutputArguments {
                            ids: response
                                .payload
                                .ids
                                .ok_or(ListActionInvoker::get_err("ids"))?,
                        },
                        content_type: response.content_type,
                        format_indicator: response.format_indicator,
                        custom_user_data: response.custom_user_data,
    ...
    /// Shutdown the [`ListActionInvoker`]. Unsubscribes from the response topic and cancels the receiver loop to drop the receiver and to prevent the task from looping indefinitely.
    ///
    ...
    fn get_err(field_name: &str) -> AIOProtocolError {
        AIOProtocolError {
            ...
            property_name: None,
            property_value: None,
            command_name: Some("list".to_string()),
