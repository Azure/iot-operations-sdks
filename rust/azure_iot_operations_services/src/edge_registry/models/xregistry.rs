...
fn labels_to_gen(labels: HashMap<String, String>) -> Vec<client_gen::Label> {
    labels
        .into_iter()
...
/// Converts a generated map of base64 extension values into raw byte vectors.
fn extensions_from_gen(extensions: HashMap<String, Bytes>) -> HashMap<String, Vec<u8>> {
    extensions.into_iter().map(|(k, v)| (k, v.0)).collect()
}

/// Converts a map of raw byte extension values into the generated base64 type.
fn extensions_to_gen(extensions: HashMap<String, Vec<u8>>) -> HashMap<String, Bytes> {
    extensions.into_iter().map(|(k, v)| (k, Bytes(v))).collect()
}
...
impl From<GroupAttributes> for client_gen::GroupAttributes {
    fn from(value: GroupAttributes) -> Self {
...
impl From<VersionAttributes> for client_gen::VersionAttributes {
    fn from(value: VersionAttributes) -> Self {
        client_gen::VersionAttributes {
            ...
            content_type: value.content_type,
            format: value.format,
            document: value.document.map(Bytes),
            extensions: extensions_to_gen(value.extensions),
        }
    }
}
...
impl From<CreateResourceRequest> for client_gen::CreateResourceRequestPayload {
    fn from(value: CreateResourceRequest) -> Self {
        client_gen::CreateResourceRequestPayload {
            group_id: value.group_id,
            meta: client_gen::ResourceMetaAttributes::from(value.meta),
            default_version: client_gen::VersionAttributes::from(value.default_version),
            extensions: extensions_to_gen(value.extensions),
        }
    }
}
...
impl From<CreateVersionRequest> for client_gen::CreateVersionRequestPayload {
    fn from(value: CreateVersionRequest) -> Self {
        client_gen::CreateVersionRequestPayload {
            group_id: value.group_id,
