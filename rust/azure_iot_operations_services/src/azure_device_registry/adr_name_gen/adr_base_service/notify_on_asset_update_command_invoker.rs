...
use super::notify_on_asset_update_request_payload::NotifyOnAssetUpdateRequestPayload;
use super::notify_on_asset_update_response_payload::NotifyOnAssetUpdateResponsePayload;

pub type NotifyOnAssetUpdateRequest =
    rpc_command::invoker::Request<NotifyOnAssetUpdateRequestPayload>;
pub type NotifyOnAssetUpdateResponse =
    rpc_command::invoker::Response<NotifyOnAssetUpdateResponsePayload>;
pub type NotifyOnAssetUpdateRequestBuilderError = rpc_command::invoker::RequestBuilderError;

#[derive(Default)]
/// Builder for [`NotifyOnAssetUpdateRequest`]
pub struct NotifyOnAssetUpdateRequestBuilder {
    inner_builder: rpc_command::invoker::RequestBuilder<NotifyOnAssetUpdateRequestPayload>,
    topic_tokens: HashMap<String, String>,
}

impl NotifyOnAssetUpdateRequestBuilder {
    ...
    pub fn payload(
        &mut self,
        payload: NotifyOnAssetUpdateRequestPayload,
    ) -> Result<&mut Self, AIOProtocolError> {
        self.inner_builder.payload(payload)?;
        Ok(self)
    }

    /// Builds a new `NotifyOnAssetUpdateRequest`
    ///
    /// # Errors
    ...
    pub fn build(
        &mut self,
    ) -> Result<NotifyOnAssetUpdateRequest, NotifyOnAssetUpdateRequestBuilderError> {
...
/// Command Invoker for `notifyOnAssetUpdate`
pub struct NotifyOnAssetUpdateCommandInvoker<C>(
    rpc_command::Invoker<NotifyOnAssetUpdateRequestPayload, NotifyOnAssetUpdateResponsePayload, C>,
)
where
    C: ManagedClient + Clone + Send + Sync + 'static,
    C::PubReceiver: Send + Sync + 'static;

impl<C> NotifyOnAssetUpdateCommandInvoker<C>
where
    C: ManagedClient + Clone + Send + Sync + 'static,
    C::PubReceiver: Send + Sync + 'static,
{
    /// Creates a new [`NotifyOnAssetUpdateCommandInvoker`]
    ///
    /// # Panics
    ...
    pub fn new(
        ...
        topic_token_map.insert("commandName".to_string(), "notifyOnAssetUpdate".to_string());

        let invoker_options = invoker_options_builder
            .request_topic_pattern(REQUEST_TOPIC_PATTERN)
            .command_name("notifyOnAssetUpdate")
    ...
    /// Invokes the [`NotifyOnAssetUpdateRequest`]
    ///
    /// # Errors
    /// [`AIOProtocolError`] if there is a failure invoking the request
    pub async fn invoke(
        &self,
        request: NotifyOnAssetUpdateRequest,
    ) -> Result<NotifyOnAssetUpdateResponse, AIOProtocolError> {
        self.0.invoke(request).await
    }

    /// Shutdown the [`NotifyOnAssetUpdateCommandInvoker`]. Unsubscribes from the response topic and cancels the receiver loop to drop the receiver and to prevent the task from looping indefinitely.
    ///
