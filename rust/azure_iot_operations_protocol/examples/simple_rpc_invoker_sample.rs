...
async fn invoke_loop(
    ...
    let incr_invoker_options = invoker::OptionsBuilder::default()
        .request_topic_pattern(REQUEST_TOPIC_PATTERN)
        .response_topic_pattern(RESPONSE_TOPIC_PATTERN.to_string())
        .command_name("increment")
        .build()
        .unwrap();
    let incr_invoker: Invoker<IncrRequestPayload, IncrResponsePayload, _> =
        Invoker::new(application_context, client, incr_invoker_options).unwrap();

    // Send 10 increment requests
    for i in 1..11 {
        let payload = invoker::RequestBuilder::default()
