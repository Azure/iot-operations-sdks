...
pub trait DataTransformer {
  ...
  fn add_sampled_data(&self, data: Data, datapoint: Option<AssetDatasetDataPoint>) -> impl std::future::Future<Output = Result<CompletionToken, String>> + Send;
}
...
impl DefaultDataTransformer {
  #[must_use] pub fn new(_dataset_definition: AssetDataset, forwarder: Forwarder, _reporter: Arc<Reporter>) -> Self {
    Self {
      forwarder,
    }
  }
  ...
  /// # Errors
  /// TODO
  pub async fn add_sampled_data(&self, data: Data, _datapoint: Option<AssetDatasetDataPoint>) -> Result<CompletionToken, String> {
