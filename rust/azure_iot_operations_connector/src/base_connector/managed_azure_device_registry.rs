...
use azure_iot_operations_services::azure_device_registry::{Asset, AssetDataset, AssetDatasetDataPoint, AssetUpdateObservation, ConfigError, Device, DeviceUpdateObservation, MessageSchemaReference};
...
impl ManagedDeviceCreateObservation {
  /// Creates a new [`ManagedDeviceCreateObservation`] that uses the given [`ConnectorContext`]
    ...
    pub async fn recv_notification(&self) -> Option<(ManagedDeviceEndpoint, ManagedDeviceEndpointUpdateObservation, /*DeviceDeleteToken,*/ ManagedAssetCreateObservation)> {
...
impl ManagedDeviceEndpoint {
  ...
  pub fn report_status(_device_status: Result<(), ConfigError>, _endpoint_status: Result<(), ConfigError>) {
...
impl ManagedAssetCreateObservation {
  ...
  #[allow(clippy::unused_async)]
  pub async fn recv_notification(&self) -> Option<(ManagedAsset, ManagedAssetUpdateObservation, AssetDeletionToken)> {
    // handle the notification
...
impl<T> ManagedDataset<T> where T: DataTransformer {
  ...
  /// # Errors
  /// TODO
  pub async fn add_sampled_data(&self, data: Data, datapoint: Option<AssetDatasetDataPoint>) -> Result<CompletionToken, String> {
    // Add sampled data to the dataset
    self.data_transformer.add_sampled_data(data, datapoint).await
  }
}
...
impl Reporter {
  ...
  /// # Errors
  /// TODO
  pub fn report_status(&self, _status: Result<Option<MessageSchema>, ConfigError>) -> Result<Option<MessageSchemaReference>, String> {
    // Report the status of the dataset
    Ok(None)
  }

  /// Returns the current message schema URI
  #[must_use] pub fn get_current_message_schema_uri(&self) -> Option<MessageSchemaReference> {
