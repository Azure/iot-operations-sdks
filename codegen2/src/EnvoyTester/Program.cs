namespace EnvoyTester
{
    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            var (targetLanguage, extension) = args[3].ToLower() switch
            {
                "c#" => (TargetLanguage.CSharp, "cs"),
                "rust" => (TargetLanguage.Rust, "rs"),
            ...
            foreach (KeyValuePair<string, string> modelNameAndText in modelTextsByName)
            {
                ...
                foreach (GeneratedItem genEnvoy in EnvoyGenerator.GenerateEnvoys(thing, schemaNamer, targetLanguage, "Namespace", projectName, "MyService", sdkPath, typeFileNames))
                {
