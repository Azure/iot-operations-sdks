namespace Azure.Iot.Operations.EnvoyGenerator
{
    ...
    public static class EnvoyGenerator
    {
        ...
        {
            EnvoyTransformFactory envoyFactory = new(targetLanguage, new CodeName(genNamespace), projectName, new CodeName(genNamespace), srcSubdir, generateClient, generateServer);
            ...
            {
                ActionEnvoyGenerator.GenerateActionEnvoys(parsedThing.Thing, parsedThing.SchemaNamer, envoyFactory, transforms, errorSpecs, typesToSerialize);
                EventEnvoyGenerator.GenerateEventEnvoys(parsedThing.Thing, parsedThing.SchemaNamer, envoyFactory, transforms, typesToSerialize);
            ...
            foreach (IEnvoyTemplateTransform service in envoyFactory.GetServiceTransforms(transforms.Keys.Concat(typeFileNames).ToList()))
            {
            ...
            foreach (IEnvoyTemplateTransform project in envoyFactory.GetProjectTransforms(serializationFormats, sdkPath, generateProject))
            {
