...
{
    public class PollingTelemetryConnectorWorker : BackgroundService
    {
        private readonly ILogger<PollingTelemetryConnectorWorker> _logger;
        ...
        public PollingTelemetryConnectorWorker(
            ILogger<PollingTelemetryConnectorWorker> logger,
        ...
        private async void SampleDataset(object? status)
        {
            ...
            { 
                ...
                {
                    _logger.LogWarning($"Dataset with name {datasetName} in asset with name {assetName} has no configured MQTT topic to publish to. This sample won't publish the data sampled from the asset.");
                    return;
                }
