    ...
    public class StateStoreClient : IAsyncDisposable, IStateStoreClient
    {
        ...
        private async Task OnTelemetryReceived(MqttApplicationMessageReceivedEventArgs args)
        {
            ...
            if (MqttTopicProcessor.DoesTopicMatchFilter(topic, string.Format(NotificationsTopicFilter, _clientIdHexString)))
            {
                ...
                {
                    _applicationContext.Logger?.LogWarning("Received a message on the key-notify topic without any payload. Ignoring it.");
                    return;
                }
                ...
                {
                    _applicationContext.Logger?.LogWarning("Received a message on the key-notify topic with an unexpected topic format. Ignoring it.");
                    return;
                }
                ...
                {
                    _applicationContext.Logger?.LogWarning("Received a message on the key-notify topic with an unexpected topic format. Could not decode the key that was notified. Ignoring it.", ex);
                    return;
                }

                if (version == null)
                {
                    _applicationContext.Logger?.LogWarning("Received a message on the key-notify topic without a timestamp. Ignoring it");
                    return;
                }
                ...
                try
                {
                    ...
                    {
                        _applicationContext.Logger?.LogWarning("Received a message on the key-notify topic with no payload. Ignoring it.");
                        return;
                    }
                ...
                {
                    _applicationContext.Logger?.LogWarning("Received a message on the key-notify topic with an unexpected payload format. Ignoring it.", ex);
                    return;
                }
        ...
        public virtual async Task<StateStoreGetResponse> GetAsync(StateStoreKey key, TimeSpan? requestTimeout = null, CancellationToken cancellationToken = default)
        {
            ...
            _applicationContext.Logger?.LogInformation($"GET {Encoding.ASCII.GetString(key.Bytes)}");
        ...
        public virtual async Task<StateStoreSetResponse> SetAsync(StateStoreKey key, StateStoreValue value, StateStoreSetRequestOptions? options = null, TimeSpan? requestTimeout = null, CancellationToken cancellationToken = default)
        {
            ...
            _applicationContext.Logger?.LogInformation($"SET {Encoding.ASCII.GetString(key.Bytes)}");
        ...
        public virtual async Task<StateStoreDeleteResponse> DeleteAsync(StateStoreKey key, StateStoreDeleteRequestOptions? options = null, TimeSpan? requestTimeout = null, CancellationToken cancellationToken = default)
        {
            ...
            _applicationContext.Logger?.LogInformation($"DEL {Encoding.ASCII.GetString(key.Bytes)}");
        ...
        public virtual async Task ObserveAsync(StateStoreKey key, TimeSpan? requestTimeout = null, CancellationToken cancellationToken = default)
        {
            ...
            if (!_isSubscribedToNotifications)
            {
                ...
                _applicationContext.Logger?.LogInformation($"Subscribed to key notifications for this client");
            }

            _applicationContext.Logger?.LogInformation($"OBSERVE {Encoding.ASCII.GetString(key.Bytes)}");
        ...
        public async virtual Task UnobserveAsync(StateStoreKey key, TimeSpan? requestTimeout = null, CancellationToken cancellationToken = default)
        {
            ...
            _applicationContext.Logger?.LogInformation($"UNOBSERVE {Encoding.ASCII.GetString(key.Bytes)}");
            ...
            if (_generatedClientStub != null && _mqttClient != null)
            {
                    ...
                    && !string.IsNullOrEmpty(_mqttClient.ClientId))
                {
                    ...
                    catch (Exception e)
                    {
                        ...
                        _applicationContext.Logger?.LogWarning("Failed to unsubscribe from key notifications MQTT topic.", e);
                    }
                }
