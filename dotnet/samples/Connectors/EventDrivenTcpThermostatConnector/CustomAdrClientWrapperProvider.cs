...
{
    internal class CustomAdrClientWrapperProvider : IAzureDeviceRegistryClientWrapperProvider
    {
        ...
        public IAzureDeviceRegistryClientWrapper CreateAdrClientWrapper(ApplicationContext applicationContext, IMqttPubSubClient mqttPubSubClient)
        {
            ...
            // for file changes
            return new AzureDeviceRegistryClientWrapper(new AzureDeviceRegistryServiceClient(applicationContext, mqttPubSubClient), new AssetFileMonitor(new PollingFilesMonitorFactory(TimeSpan.FromMilliseconds(500))));
        }
    }
}
