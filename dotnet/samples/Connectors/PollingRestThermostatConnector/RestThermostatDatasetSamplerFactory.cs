...
namespace RestThermostatConnector
{
    public class RestThermostatDatasetSamplerFactory : IDatasetSamplerFactory
    {
        ...
        public IDatasetSampler CreateDatasetSampler(Device device, string inboundEndpointName, Asset asset, AssetDataset dataset, EndpointCredentials? credentials)
        {
            if (dataset.Name.Equals("thermostat_status"))
            {
                    ...
                    && device.Endpoints.Inbound.TryGetValue(inboundEndpointName, out var inboundEndpoint))
                {
                    ...
                    return new ThermostatStatusDatasetSampler(httpClient, asset.ExternalAssetId!, credentials);
                }
            }

            throw new InvalidOperationException($"Unrecognized dataset with name {dataset.Name} on asset with name {asset.ExternalAssetId}");
        }
    }
}
