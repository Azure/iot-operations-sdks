...
namespace Azure.Iot.Operations.Protocol.UnitTests.Chunking
{
    public class ChunkingMqttClientTests
    {
        ...
        public async Task PublishAsync_SmallMessage_PassesThroughToInnerClient()
        {
            ...
            if (mockInnerClient.Object.ConnectedAsync != null)
            {
                await mockInnerClient.Object.ConnectedAsync.Invoke(connectedArgs);
            }

            // Create a small message that doesn't need chunking
            var smallPayload = new byte[100];
            var smallMessage = new MqttApplicationMessage("test/topic", MqttQualityOfServiceLevel.AtLeastOnce)
            {
        ...
        public async Task PublishAsync_LargeMessage_ChunksMessageAndSendsMultipleMessages()
        {
            ...
            var connectResult = new MqttClientConnectResult(
                true,
                MqttClientConnectReasonCode.Success,
                null,
                maxPacketSize,
                null,
                null,
            ...
            var largeMessage = new MqttApplicationMessage("test/topic", MqttQualityOfServiceLevel.AtLeastOnce)
            {
        ...
        public async Task HandleApplicationMessageReceivedAsync_NonChunkedMessage_PassesThroughToHandler()
        {
            ...
            var message = new MqttApplicationMessage("test/topic", MqttQualityOfServiceLevel.AtLeastOnce)
            {
            ...
            if (mockInnerClient.Object.ApplicationMessageReceivedAsync != null)
            {
                await mockInnerClient.Object.ApplicationMessageReceivedAsync.Invoke(receivedArgs);
            }
        ...
        public async Task HandleApplicationMessageReceivedAsync_ChunkedMessage_ReassemblesBeforeDelivering()
        {
            ...
            if (mockInnerClient.Object.ApplicationMessageReceivedAsync != null)
            {
                await mockInnerClient.Object.ApplicationMessageReceivedAsync.Invoke(receivedArgs1);
                await mockInnerClient.Object.ApplicationMessageReceivedAsync.Invoke(receivedArgs2);
            }
        ...
        [Fact]
        public void DisconnectedAsync_ClearsInProgressChunks()
        {
            ...
            // Create and setup a disconnect event
            var disconnectArgs = new MqttClientDisconnectedEventArgs(
                true,
                MqttClientDisconnectReasonCode.NormalDisconnection,
                string.Empty,
                null,
                0,
                new List<MqttUserProperty>());
            ...
            // Act
            if (mockInnerClient.Object.DisconnectedAsync != null)
            {
                mockInnerClient.Object.DisconnectedAsync.Invoke(disconnectArgs);
            }
            ...
            mockInnerClient.Verify(client => client.DisconnectedAsync, Times.AtLeastOnce());
        }
