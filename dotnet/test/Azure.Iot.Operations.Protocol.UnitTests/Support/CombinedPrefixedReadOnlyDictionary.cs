...
namespace Azure.Iot.Operations.Protocol.UnitTests.Support
{
    ...
    public class CombinedPrefixedReadOnlyDictionary<TValue> : IReadOnlyDictionary<string, TValue>
    {
            ...
            IReadOnlyDictionary<string, TValue> dict2)
        {
            ...
            this._prefix1 = prefix1;
            this._dict1 = dict1;
            this._prefix2 = prefix2;
            this._dict2 = dict2;
        }

        /// <inheritdoc/>
        IEnumerable<string> IReadOnlyDictionary<string, TValue>.Keys => this._dict1.Keys.Select(k => $"{this._prefix1}{k}").Concat(this._dict2.Keys.Select(k => $"{this._prefix2}{k}"));

        /// <inheritdoc/>
        IEnumerable<TValue> IReadOnlyDictionary<string, TValue>.Values => this._dict1.Values.Concat(this._dict2.Values);

        /// <inheritdoc/>
        int IReadOnlyCollection<KeyValuePair<string, TValue>>.Count => this._dict1.Count + this._dict2.Count;

        /// <inheritdoc/>
        TValue IReadOnlyDictionary<string, TValue>.this[string key] =>
            key.StartsWith(this._prefix1, StringComparison.InvariantCulture) && this._dict1.TryGetValue(key.Substring(this._prefix1.Length), out TValue? value1) ? value1 :
            key.StartsWith(this._prefix2, StringComparison.InvariantCulture) && this._dict2.TryGetValue(key.Substring(this._prefix2.Length), out TValue? value2) ? value2 :
            default(TValue)!;
        ...
        {
            return
                key.StartsWith(this._prefix1, StringComparison.InvariantCulture) && this._dict1.ContainsKey(key.Substring(this._prefix1.Length)) ||
                key.StartsWith(this._prefix2, StringComparison.InvariantCulture) && this._dict2.ContainsKey(key.Substring(this._prefix2.Length));
        }

        /// <inheritdoc/>
        IEnumerator<KeyValuePair<string, TValue>> IEnumerable<KeyValuePair<string, TValue>>.GetEnumerator()
        {
            foreach (var item in this._dict1)
            {
                yield return new KeyValuePair<string, TValue>($"{this._prefix1}{item.Key}", item.Value);
            }

            foreach (var item in this._dict2)
            {
                yield return new KeyValuePair<string, TValue>($"{this._prefix2}{item.Key}", item.Value);
            }
        }
        ...
        {
            if (key.StartsWith(this._prefix1, StringComparison.InvariantCulture) && this._dict1.TryGetValue(key.Substring(this._prefix1.Length), out TValue? value1))
            {
                value = value1;
                return true;
            }

            if (key.StartsWith(this._prefix2, StringComparison.InvariantCulture) && this._dict2.TryGetValue(key.Substring(this._prefix2.Length), out TValue? value2))
            {
                value = value2;
                return true;
            }
