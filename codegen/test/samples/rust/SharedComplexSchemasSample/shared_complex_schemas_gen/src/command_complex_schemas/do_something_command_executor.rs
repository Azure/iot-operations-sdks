...
use super::super::shared_schemas::some_response_info::SomeResponseInfo;
use super::do_something_request_payload::DoSomethingRequestPayload;
use super::MODEL_ID;
use super::REQUEST_TOPIC_PATTERN;

pub type DoSomethingRequest = CommandRequest<DoSomethingRequestPayload, SomeResponseInfo>;
pub type DoSomethingResponse = CommandResponse<SomeResponseInfo>;
pub type DoSomethingResponseBuilderError = CommandResponseBuilderError;

/// Builder for [`DoSomethingResponse`]
#[derive(Default)]
pub struct DoSomethingResponseBuilder {
    inner_builder: CommandResponseBuilder<SomeResponseInfo>,
}

impl DoSomethingResponseBuilder {
    ...
    pub fn payload(&mut self, payload: SomeResponseInfo) -> Result<&mut Self, AIOProtocolError> {
        self.inner_builder.payload(payload)?;
        Ok(self)
    }

    /// Builds a new `DoSomethingResponse`
    ///
    /// # Errors
    /// If a required field has not been initialized
    #[allow(clippy::missing_panics_doc)] // The panic is not possible
    pub fn build(&mut self) -> Result<DoSomethingResponse, DoSomethingResponseBuilderError> {
        self.inner_builder.build()
    }
}

/// Command Executor for `doSomething`
pub struct DoSomethingCommandExecutor<C>(
    CommandExecutor<DoSomethingRequestPayload, SomeResponseInfo, C>,
)
where
    C: ManagedClient + Clone + Send + Sync + 'static,
    C::PubReceiver: Send + Sync + 'static;

impl<C> DoSomethingCommandExecutor<C>
where
    C: ManagedClient + Clone + Send + Sync + 'static,
    C::PubReceiver: Send + Sync + 'static,
{
    /// Creates a new [`DoSomethingCommandExecutor`]
    ///
    /// # Panics
    ...
    pub fn new(
        ...
        topic_token_map.insert("commandName".to_string(), "doSomething".to_string());
    ...
    /// Receive the next [`DoSomethingRequest`] or [`None`] if there will be no more requests
    ///
    /// # Errors
    /// [`AIOProtocolError`] if there is a failure receiving a request
    pub async fn recv(&mut self) -> Option<Result<DoSomethingRequest, AIOProtocolError>> {
        self.0.recv().await
    }

    /// Shutdown the [`DoSomethingCommandExecutor`]. Unsubscribes from the request topic.
    ///
