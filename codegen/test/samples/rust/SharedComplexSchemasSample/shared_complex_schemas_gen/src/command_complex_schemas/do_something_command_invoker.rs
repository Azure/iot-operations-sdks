...
pub type DoSomethingRequest = CommandRequest<DoSomethingRequestPayload>;
pub type DoSomethingResponse = CommandResponse<SomeResponseInfo>;
pub type DoSomethingRequestBuilderError = CommandRequestBuilderError;

#[derive(Default)]
/// Builder for [`DoSomethingRequest`]
pub struct DoSomethingRequestBuilder {
    inner_builder: CommandRequestBuilder<DoSomethingRequestPayload>,
}

impl DoSomethingRequestBuilder {
    ...
    pub fn payload(
        &mut self,
        payload: DoSomethingRequestPayload,
    ) -> Result<&mut Self, AIOProtocolError> {
        self.inner_builder.payload(payload)?;
        Ok(self)
    }

    /// Builds a new `DoSomethingRequest`
    ///
    /// # Errors
    /// If a required field has not been initialized
    #[allow(clippy::missing_panics_doc)] // The panic is not possible
    pub fn build(&mut self) -> Result<DoSomethingRequest, DoSomethingRequestBuilderError> {
        self.inner_builder.build()
    }
}

/// Command Invoker for `doSomething`
pub struct DoSomethingCommandInvoker<C>(
    CommandInvoker<DoSomethingRequestPayload, SomeResponseInfo, C>,
)
where
    C: ManagedClient + Clone + Send + Sync + 'static,
    C::PubReceiver: Send + Sync + 'static;

impl<C> DoSomethingCommandInvoker<C>
where
    C: ManagedClient + Clone + Send + Sync + 'static,
    C::PubReceiver: Send + Sync + 'static,
{
    /// Creates a new [`DoSomethingCommandInvoker`]
    ///
    /// # Panics
    ...
    /// Invokes the [`DoSomethingRequest`]
    ///
    /// # Errors
    ...
    ) -> Result<DoSomethingResponse, AIOProtocolError> {
        self.0.invoke(request).await
    }

    /// Shutdown the [`DoSomethingCommandInvoker`]. Unsubscribes from the response topic and cancels the receiver loop to drop the receiver and to prevent the task from looping indefinitely.
    ///
